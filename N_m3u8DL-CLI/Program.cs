...
namespace N_m3u8DL_CLI.NetCore
{
    class Program
    {
        ...
        private static string reqHeaders = "";
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine("输入list.txt可以直接批量解析、下载目录下的list.txt(文件编码为utf-8)中的m3u8列表");
                ...
                if (cmd.EndsWith("list.txt"))//批量模式
                {
                    ...
                    {
                        listFileName = Path.Combine(Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName), cmd);
                    }
                    if (File.Exists(listFileName))
                    {
                        var cmds = File.ReadAllLines(listFileName);
                        fileCount = cmds.Length;
                        listCount = cmds.Length;
                        foreach (var item in cmds)
                        {
                            args = Global.ParseArguments(item).ToArray();  //解析命令行
                            Global.WriteInit();
